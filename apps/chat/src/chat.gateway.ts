...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  private async createConversations(socket: Socket, userId: number) {
    const ob2$ = this.authService.send({ cmd: 'get-friends-list' }, { userId });
  ...
  async handleMessage(socket: Socket, createMessageDto: CreateMessageDto) {
    ...
    const friendsDetails = (await this.cache.get(
      `conversationUser ${createMessageDto.friendId}`,
